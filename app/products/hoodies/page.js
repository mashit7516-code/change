        ...
        const findproducts = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    const filtered = data.filter(p => p.catogery === "hoodies")
                    console.log("Filtered:", filtered)
                    setproduct(filtered)
                    
                    if (filtered.length === 0) {
                        seterror(true)
                        seterrormessage("No hoodies found")
                    }
                } else if (data.success === false) {
                    seterror(true)
                    seterrormessage(data.message || "Failed to load products")
                } else if (data.products && Array.isArray(data.products)) {
                    const filtered = data.products.filter(p => p.catogery === "hoodies")
                    setproduct(filtered)
                ...
                <div className='w-full'>
                    <div className='flex flex-col items-center justify-center gap-4 mb-8'>
                        ...
                        <span className='font-semibold text-center'>
                           Soft, cozy, and built for everyday wear — our hoodies deliver warmth with a clean modern style. Change – Brings Comfort makes sure you feel relaxed, confident, and comfortable wherever you go.</span>
                    </div>
                    <div className="products m-2 border-white grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 border-2 rounded-lg bg-[rgba(255,255,255,0.2)] p-4">
                        {product.map((item, index) => (
                            <Link href={`/products/hoodies/${item._id}`} key={item._id || item.id || index} className="border rounded-lg p-4 bg-[rgba(255,255,255,0.2)]">
                                <div className="image">
                                    <Image 
                                        width={200} 
                                        height={200} 

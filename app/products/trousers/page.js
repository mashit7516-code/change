        ...
        const findproducts = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    const filtered = data.filter(p => p.catogery === "trousers")
                    console.log("Filtered:", filtered)
                    setproduct(filtered)
                    
                    if (filtered.length === 0) {
                        seterror(true)
                        seterrormessage("No trousers found")
                    }
                } else if (data.success === false) {
                    seterror(true)
                    seterrormessage(data.message || "Failed to load products")
                } else if (data.products && Array.isArray(data.products)) {
                    const filtered = data.products.filter(p => p.catogery === "trousers")
                    setproduct(filtered)
                ...
                <div className='w-full'>
                    <div className='flex flex-col items-center justify-center gap-4 mb-8'>
                        ...
                        <span className='font-semibold text-center'>
                           Our trousers bring a polished yet relaxed feel. Designed with premium stitching, smooth fabric, and a fit that stays sharp — Change ensures comfort meets class in every pair.</span>
                    </div>
